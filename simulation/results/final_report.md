# Phase 2 シミュレーション 最終パラメータ推奨レポート
...
| D軸（距離軸） | **不使用（Phase 2）** | T010 で有効性に一貫性なし |
...
実際の地理情報・GPS データがあれば再評価を推奨。Phase 2 は D=0.5 固定でよい。
...
| 距離軸の要否を結論付ける | ✅ **結論** | Phase 2 では不使用（3層で進める） |
